...
A project Claude Code skill wraps the Agent API so agents can ping the user:
`.claude/skills/maomao-notify/` (SKILL.md + notify.py, zero deps; tracked with
the repo). Reads `MAOMAO_URL` + `MAOMAO_TOKEN` (an agent token with
`notification:send`). Usage:
`python3 .claude/skills/maomao-notify/notify.py "訊息"` (flags: --title,
